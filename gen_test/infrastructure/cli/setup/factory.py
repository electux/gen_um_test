...
from gen_test.infrastructure.cli.setup.keys import CLIBundleKeys
from gen_test.infrastructure.cli.setup.opt_validator import CLIBundleOptionsValidator
...
class CLIBundleFactory:
    ...
    def create_bundle(cls, options: CLIBundleOptions) -> CLIBundle:
        ...
        suite_cmd = CommandBundle(definition=suite_def, executor=SuiteCommandExecutor(definition=suite_def))

        mock_def = MockCommandDefinition()
        mock_cmd = CommandBundle(definition=mock_def, executor=MockCommandExecutor(definition=mock_def))

        test_def = TestCommandDefinition()
        test_cmd = CommandBundle(definition=test_def, executor=TestCommandExecutor(definition=test_def))

        fake_def = FakeCommandDefinition()
        fake_cmd = CommandBundle(definition=fake_def, executor=FakeCommandExecutor(definition=fake_def))

        cmake_def = CMakeCommandDefinition()
        cmake_cmd = CommandBundle(definition=cmake_def, executor=CMakeCommandExecutor(definition=cmake_def))

        commands = [suite_cmd, mock_cmd, test_cmd, fake_cmd, cmake_cmd]
