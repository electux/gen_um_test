...
Info
    Defines SuiteCommandExecutor class.
'''
...
from gen_test.infrastructure.command.icommand_definition import ICommandDefinition
...
class SuiteCommandExecutor:
    '''
        Command executor strategy for generating full test suites.
    ...
    def __init__(self, definition: ICommandDefinition) -> None:
        '''
            Initializes SuiteCommandExecutor.
    ...
    def execute(self, *, params: Mapping[str, object], service: IService) -> Mapping[str, object]:
        ...
        config = TestSuiteConfig(
            ...
            cmake_target=str(params.get('target')) if params.get('target') else None
        )

        res = service.generate_suite(config)
        return {
